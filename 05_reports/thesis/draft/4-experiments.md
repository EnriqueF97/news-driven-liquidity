...
The LLM-extracted features are the substrate of all Phase 2 modeling, so they are validated before use. Following Section 3.7, we run an **inter-model calibration** [CITE: Gilardi et al. 2023; Zheng et al. 2023]: the same 30 stratified articles are scored by Claude Haiku 4.5 and by a GPT-family reference model with the identical prompt and schema, treating cross-family disagreement as a signal of genuine ambiguity or systematic bias. This subsection reports the first calibration, on the initial Schema v1, and the diagnostic it prompted.
