...
The categorical label, all three class probabilities, and the confidence value are retained for downstream use (persisted in the Phase 1 feature table `gdelt_wti_sentiment.csv`).
...
where `P(negative)_t` and `P(positive)_t` are the FinBERT title-plus-body class probabilities for the article published at hour `t`. The neutral probability is the omitted reference: the three class probabilities sum to one, so including all three would be perfectly collinear. The coefficients `β_1` and `β_2` therefore measure the expected difference in `log_volume` at hour `t + k` for a maximally confident bearish or bullish article (class probability equal to one), respectively, relative to a neutral one. Using the two class probabilities rather than a single signed sentiment regressor is essential to the asymmetry question. A signed specification would force the model to assume that the bearish coefficient is the negative of the bullish coefficient (a symmetric response); the two-probability specification allows the two coefficients to vary independently, which is precisely what RQ2 requires. This continuous encoding is the primary Phase 1 specification. It generalises the earlier two-dummy encoding (which shares the same two-coefficient structure but with hard 0/1 indicators) by weighting each article by FinBERT's confidence, so that a 0.95-bearish article contributes more than a 0.51-bearish one; the dummy version is retained as a baseline in Section 4.2.3.
